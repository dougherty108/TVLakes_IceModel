###### Climatological forecasting — Lake Hoare (LH) ########
...
# Lake Hoare from a MEAN ANNUAL CLIMATOLOGY — i.e. for each (day-of-year,
# hour) it averages that same timestep-of-year across every year in LH's
...
#
# This is the same approach used for ELB/WLB/LF (replacing an earlier
...
# we can see how LH ice thickness responds to those scenarios.
#
...
# machinery lives there; only LH-specific choices are made here.
...
# model on. Adjust `n_years` in 00_LH_data_preparation.R to change how many
# years the ice MODEL runs for — that is independent of the long climatology
# record built below.
source("R/LH/00_LH_data_preparation.R")
...
# so it pulls and gap-fills LH's entire available station record — using
