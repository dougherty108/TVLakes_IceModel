...
HOEM <- read_csv("~/Library/CloudStorage/OneDrive-UCB-O365/Documents/MCM-LTER_Met/met stations/mcmlter-clim_hoem_15min-20250205.csv") |> 
  ...
  filter(date_time > '2016-12-14 00:00:00') |> 
...
###################### Define Parameters ######################
L_initial <- 3.5       # Initial ice thickness (m) Ice thickness at 12/17/2016 ice to ice
...
# the Lake Hoare Blue Box station (LHBB). 
...
start_time <- min(HOEM$date_time)

# Generate model time steps (POSIXct format)
time_model <- start_time + seq(0, by = dt * 86400, length.out = nt)  # Convert dt from days to seconds


air_temperature <- read_csv("Data/air_temp_LHBB.csv") |> 
  mutate(date_time = mdy_hm(date_time), 
         airtemp_3m_K = surftemp_degC + 273.15)

# load air temperature data from the Lake Fryxell Lake Monitoring Station, to fill gaps in the LHBB record
lfbb_airtemp <- read_csv('Data/air_temp_LFBB.csv') |> 
  mutate(date_time = mdy_hm(date_time), 
         airtemp_3m_K = surftemp_degc + 273.15) |> 
...
air_temperature <- air_temp_gaps |> 
  mutate(airtemp_3m_K = ifelse(is.na(airtemp_3m_K), lfbb_airtemp$airtemp_3m_K, airtemp_3m_K))
...
shortwave_radiation_initial <- HOEM |> 
  dplyr::select(metlocid, date_time, swradin_wm2) |> 
  mutate(swradin_wm2 = ifelse(is.na(swradin_wm2), FRLM$swradin_wm2, swradin_wm2)) # replace empty shortwave data with FRLM, nearest met station
...
wind_speed = HOEM |> 
  ...
  mutate(wspd_ms = ifelse(is.na(wspd_ms), FRLM$wspd_ms, wspd_ms)) # fill in lost wind values from TARM, next nearest met station


###################### RELATIVE HUMIDITY DATA ######################
# load relative humidity data
relative_humidity <- HOEM |> 
  dplyr::select(metlocid, date_time, rhh2o_3m_pct, rhice_3m_pct) |> 
  mutate(rhh2o_3m_pct = ifelse(is.na(rhh2o_3m_pct), FRLM$rhh2o_3m_pct, rhh2o_3m_pct))
