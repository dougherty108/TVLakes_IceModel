###### Climatological forecasting — East Lake Bonney (ELB) ########
...
# East Lake Bonney from a MEAN ANNUAL CLIMATOLOGY — i.e. for each (day-of-year,
# hour) it averages that same timestep-of-year across every year in ELB's
...
#
# This replaces the earlier VAR-bootstrap forecasting approach: instead of
...
# we can see how ELB ice thickness responds to those scenarios.
#
...
# machinery lives there; only ELB-specific choices are made here.
...
# model on. Adjust `n_years` in 00_ELB_data_preparation.R to change how many
...
# so it pulls and gap-fills ELB's entire available station record — using
