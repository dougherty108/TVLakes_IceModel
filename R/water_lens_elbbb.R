...
Tf = 273.15             # Temperature of water freezing (K)
...
for (t_idx in 1:nrow(time_series)) {
  ...
  # 1D heat diffusion
  for (i in 2:(length(prevT) - 1)) {
    newT[i] <- prevT[i] + alpha * ((dt * 86400) / dx^2) * (prevT[i + 1] - 2 * prevT[i] + prevT[i - 1])
  }
  
  # Boundary conditions
  newT[1] <- T_air
  newT[length(prevT)] <- Tf  # Bottom at freezing point
  
  # === INSERT WATER LENS IN DECEMBER ===
  current_date <- as.Date(time_series$time[t_idx])
  current_month <- format(current_date, "%m")
  current_year <- format(current_date, "%Y")
  current_week <- format(current_date, "%U")
  
  lens_depth <- prevL * 0.5
  lens_thickness <- 0.1
  
  if (current_week == "0" | 
      current_week == "1" && !(current_year %in% lens_inserted_years)) {
    if (!is.na(newL) && newL > (lens_depth + lens_thickness)) {
      lens_index <- which.min(abs(depth - lens_depth))
      newT[lens_index] <- Tf  # Liquid water lens at freezing temp
      lens_inserted_years <- c(lens_inserted_years, current_year)
    }
  }
  
  # LATENT HEAT AT LENS DEPTH
  lens_index <- which.min(abs(max(depth) - lens_depth))  # Use current depth
  if (!is.na(newT[lens_index])) {
    latent_heat_lens <- ifelse(newT[lens_index] >= Tf, xLf, xLf)
    results$latent_heat_lens[t_idx] <- latent_heat_lens
  }
  ...
  # === INTERPOLATE DEPTH PROFILE ===
  if (newL > 0) {
    dx <- 0.1
    newdepth <- seq(0, newL, by = dx)
    newT <- approx(seq(0, prevL, length.out = length(depth)), newT, 
                   seq(0, newL, length.out = length(newdepth)), rule = 2)$y
  } else {
    newT <- rep(0, nx)
    depth <- NA
  }
  
  # === REINSERT WATER LENS TEMP (PRESERVE IT) ===
  if (newL > (lens_depth + lens_thickness)) {
    lens_index <- which.min(abs(newdepth - lens_depth))
    newT[lens_index] <- Tf  # Preserve liquid water temp
  }
  
  # === PREVENT BOTTOM FREEZING IF LENS IS LIQUID ===
  lens_index <- which.min(abs(newdepth - lens_depth))
  water_lens_frozen <- (newT[lens_index] <= Tf)
  results$water_lens_frozen[t_idx] <- water_lens_frozen
  
  if (!is.na(newL) && newL > 0 && water_lens_frozen) {
