...
###################### Define Parameters ######################
L_initial <- 3.39       # Initial ice thickness (m) Ice thickness at 12/17/2016 ice to ice
...
L_f <- xLf  
Chi = 0.3               # Solar Absorption constant (adustable)             
...
# the East Lake Bonney Permanent Monitoring Station (wlbbb)
...
air_temperature <- read_csv("Data/air_temp_WLBBB.csv") |> 
...
results <- tibble(
  ...
  sensible_Q = numeric(n_iterations),        # Sensible heat flux
  latent_Q = numeric(n_iterations),          # Latent Heat Flux
  conductive_Q = numeric(n_iterations),      # Conductive Heat Flux (from water column)
  surface_heat_flux = numeric(n_iterations), # Surface heat flux
